...
def draw_game():
    # 上 8割 ＋ 下 2割のパネル
    panel_h = SCREEN_H // 5        # 24
    panel_y = SCREEN_H - panel_h   # 96

    # 背景は draw() 冒頭の cls(0) に任せる
    # 下パネル
    ...
    if game_phase == 0:
        ...
        # 「Rock Paper Scissors」＋手の選択肢
        draw_centered_text(msg_y - 10, "Rock Paper Scissors", 7)
        ...
        for i, label in enumerate(HAND_LABELS):
            ...
            pyxel.text(text_x, msg_y + 2, label, 7)
        ...
        # YES / NO 選択
        draw_centered_text(msg_y - 10, "Continue?", 7)
